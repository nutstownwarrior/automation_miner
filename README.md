...
| `llm_preferences` | Reads the reasons you typed when dismissing things and generalises them into standing preferences — "nothing in the guest room" — then hides new suggestions that match one. | Hide anything without saying so. Every hidden suggestion is listed in the archive with the rule that hid it, one click undoes it, and a preference needs at least two of your own dismissals behind it. It cannot promote or reorder anything, and it never overrules something you have already accepted, dismissed or shadow-tested. |
